...
kmeans = MiniBatchKMeans(n_clusters=2000, random_state=0)
...
for count,each in enumerate(contents):
    ...
    for window, transform in get_tiles(img):
        ...
        if window.height == 256 and window.width == 256:
            ...
            B_bands = B_bands[B_bands['65'] == 2]
            B_bands_original = B_bands_original[B_bands_original['65'] == 2]
    ...
    filename = 'E:/SCC/SCC3/TYP/ml4floods/codes/models/'+str(each)+'_'+str(count)+'_water_model.sav'
